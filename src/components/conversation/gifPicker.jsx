    ...
    const timeout = setTimeout(
      async () => {
        ...
        try {
          ...
          setGifs([])
          setNext(null)
          setError(searchError.message || 'Could not load GIFs.')
        } finally {
          if (!cancelled) {
            setLoading(false)
          }
  ...
  return (
    <div className="absolute inset-x-0 bottom-full z-40 border-t border-vibe-petrol/10 bg-vibe-bg shadow-2xl">
      ...
      <div className="h-80 overflow-y-auto px-3 py-3">
        ...
        {!loading && error && (
          <div className="flex h-full items-center justify-center px-6 text-center">
            <div>
              <p className="font-semibold text-vibe-text">Couldn't load GIFs.</p>
        ...
        {!loading && !error && gifs.length === 0 && (
          <div className="flex h-full items-center justify-center px-6 text-center">
            <p className="text-sm text-vibe-muted">No GIFs found.</p>
          </div>
        )}
        ...
        {!loading && !error && gifs.length > 0 && (
          <>
            ...
            {!next && gifs.length > 0 && <p className="py-4 text-center text-[11px] text-vibe-muted">No more GIFs</p>}
          </>
        )}
      </div>
      ...
      <div className="border-t border-vibe-petrol/10 px-4 py-2 text-center">
        <span className="text-[10px] font-semibold uppercase tracking-[0.15em] text-vibe-muted">Powered by KLIPY</span>
      </div>
    </div>
  )
}
